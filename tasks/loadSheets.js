...
module.exports = function (grunt) {
  grunt.registerTask(
    "sheets",
    "Downloads from Google Sheets -> JSON",
    async function () {
      var auth = null;
      try {
        auth = authenticate();
        ...
        console.log(
          "No access token from ~/.google_oauth_token, private spreadsheets will be unavailable.",
          err
        );
      }
      ...
      var bookRequests = sheetKeys.map(async function (spreadsheetId) {
        ...
        var sheetRequests = sheets.map(async function (sheet) {
          ...
          var filename = `data/${sheet.properties.title.replace(
            /\s+/g,
            "_"
          )}.sheet.json`;
