...
const isTesting = ["localhost", "127.0.0.1", "::1"].includes(
  ...
  window.addEventListener(
    ...
    event => {
      if (event?.data?.key ?? "" === geoEdgePolicyKey) {
        if (
          event.data &&
          Object.prototype.hasOwnProperty.call(event.data, "blocked")
        ) {
          if (event.data.blocked === true) {
            const adModel = allAdModels.find(
              model => model.id === event.data.el
            );
            SponsorshipUtil.geoedgeMessageEventCallback(event, adModel);
          } else {
            Debug.log(
              `sponsorship -> Scanned ad ID '${
                event.data.el
              }'. GE data: ${JSON.stringify(event.data)}`
            );
          }
        }
      }
...
const waitForScriptInjection = () => {
  if (
    window.NPR.scriptInjectionCompleted.aps &&
    window.NPR.scriptInjectionCompleted.openwrap
  ) {
