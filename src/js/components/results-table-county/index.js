...
class ResultsTableCounty extends ElementBase {
  ...
  async connectedCallback() {
    ...
    try {
      ...
      this.initialCandidates =
        this.race == 20978 && this.currentState === "ME"
          ? this.data[0].candidates.filter(
              (cand) => cand.last === "King" || cand.last === "Kouzounas"
            )
          : this.data[0].candidates.slice(0, 2);
      this.render();
    ...
    return `
      <span>
        <svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="10" height="16" viewBox="0 0 320 512">
          <path fill="${
            sorted && this.state.order < 0 ? "#999" : "#ddd"
  ...
  getSorter(currentState) {
    ...
    return `
      <ul class="sorter">
        <li class="label">Sort ${
          townshipStates.includes(currentState) ? "Townships" : "Counties"
        } By:</li>
  ...
  renderCountyRow(c, orderedCandidates) {
    ...
    const comparisonClass = metricValue.includes("D")
      ? "Dem"
      : metricValue.includes("R")
      ? "GOP"
      : "";
  ...
  render() {
    ...
    this.innerHTML = `
        <div class="results-counties ${this.state.sortMetric.key
          .split("_")
          .join("-")}">
            <h3 class="section-hed">Demographics by ${
              townshipStates.includes(this.currentState) ? "township" : "county"
            }</h3>
                ...
                <thead>
                    <tr>
                        <th class="county sortable" data-sort="countyName">
                            <div>
                                <span class="county">${
                                  townshipStates.includes(this.currentState)
                                    ? "Township"
                                    : "County"
                                }</span>
                            </div>
                        ...
                        <th class="comparison sortable" data-sort="${
                          this.state.displayedMetric.key
                        }">
                            <div>
                                <span class="title">${
                                  this.state.displayedMetric.name
                                }</span>
...
<tbody class="${this.state.collapsed ? "collapsed" : ""}">
                ${sortedData
                  .map((county) => {
                    const countyOrderedCandidates = orderedCandidates.map(
                      (headerCand) => {
                        if (headerCand.last === "Other") {
                          if (county.censusID) {
                            const otherCandidate = county.candidates.find(
                              (c) => c.last === "Other" || c.party === "Other"
                            );
                            return {
                              ...headerCand,
                              percent: otherCandidate?.percent || 0,
                            };
                          } else {
                        ...
                        const countyCand = county.candidates.find(
                          (c) =>
                            c.last === headerCand.last ||
                            (headerCand.last === "Other" &&
                              (c.last === "Other" || c.party === "Other"))
                    ...
                    return this.renderCountyRow(
                      county,
                      countyOrderedCandidates
                    );
                  })
                  .join("")}
            </tbody>
